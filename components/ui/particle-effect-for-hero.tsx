...
export const AntiGravityCanvas: React.FC = () => {
  ...
  const animate = useCallback((time: number) => {
    ...
    // Adjusted to match Shiden brand (Blue)
    gradient.addColorStop(0, `rgba(0, 85, 255, ${pulseOpacity})`); 
    ...
    for (let i = 0; i < particles.length; i++) {
      ...
      // Adjusted blue color to match Shiden brand better
      ctx.fillStyle = p.color === '#ffffff' 
        ? `rgba(255, 255, 255, ${opacity})` 
        : `rgba(0, 85, 255, ${opacity})`; 
  ...
  return (
    <div 
      ref={containerRef} 
      className="absolute inset-0 z-0 overflow-hidden bg-shiden-dark cursor-crosshair"
