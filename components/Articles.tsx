...
const LogoFloater: React.FC<{
    ...
    return (
        ...
        >
            ...
            <div
                className="w-full h-full rounded-2xl relative z-10 flex items-center justify-center border border-white/20 backdrop-blur-sm transition-all duration-300 group-hover:border-white/50 group-hover:shadow-[0_0_30px_rgba(255,255,255,0.1)] p-6 md:p-10 overflow-hidden"
                style={{
            ...
            >
                ...
                <div className="w-full h-full relative z-20 opacity-90 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                    <img
                        ...
                        style={{ transform: `scale(${Math.min(scale, 0.9)})` }}
                    />
                </div>
...
export const Articles: React.FC = () => {
    ...
    // Local logo images
    const LOGO_MULTIPHASE = "/partners/multiphase-digital-logo.png";
    const LOGO_KONTENT = "/img/Kontent media white logo.png";
    const LOGO_CHAKLIART = "/img/auRWRthl_400x400.png";
    ...
    return (
        <section
            ...
            id="partners"
            className="relative h-[80vh] min-h-[600px] overflow-hidden bg-[#001A29] border-t border-white/10"
        >
            <div className="absolute inset-0 perspective-[1000px]">
                ...
                {/* Clear Heading Title */}
                <div className="absolute top-8 md:top-12 left-0 w-full flex flex-col items-center justify-start z-30 pointer-events-none px-4 pb-16 md:pb-24">
                ...
                <div className="absolute bottom-0 w-[200%] -left-[50%] h-[120vh] origin-bottom transform rotate-x-[70deg] translate-z-[50px] bg-[#111] shadow-inner">
                    ...
                    <div className="absolute inset-0 bg-gradient-to-t from-[#001A29] via-transparent to-transparent"></div>
                </div>

                {/* Logos Container - EXACTLY 3 LOGOS */}
                <div className="absolute top-1/2 left-0 right-0 flex flex-col md:flex-row items-center justify-center gap-12 md:gap-24 z-20 pointer-events-none px-4 -translate-y-1/2 pt-32 md:pt-40 mt-8 md:mt-12">

                    <motion.div style={{ x: xLeft, opacity }} className="pointer-events-auto">
                        <LogoFloater logo={LOGO_MULTIPHASE} delay={0} mouseX={mouseX} mouseY={mouseY} scale={0.85} />
                    </motion.div>

                    <motion.div style={{ y: yCenter, opacity }} className="pointer-events-auto">
                        <LogoFloater logo={LOGO_KONTENT} delay={0.2} mouseX={mouseX} mouseY={mouseY} scale={0.8} />
                    </motion.div>

                    <motion.div style={{ x: xRight, opacity }} className="pointer-events-auto">
                        <LogoFloater logo={LOGO_CHAKLIART} delay={0.1} mouseX={mouseX} mouseY={mouseY} scale={0.85} />
                    </motion.div>
